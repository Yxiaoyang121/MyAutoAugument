# Validation Error Diagnosis

- Status: completed
- TP: 724
- FP: 326
- FN: 161
- Precision: 0.6895
- Recall: 0.8000

## Diagnosis Vector
- small_object_score: 0.2428
- low_contrast_score: 0.5050
- class_imbalance_score: 0.8980
- localization_score: 0.0221
- false_positive_score: 0.3105

## Issues
- low_contrast_missed_defect severity=medium suggestions=contrast, gamma, clahe, brightness, sharpen
- low_contrast_missed_defect severity=medium suggestions=contrast, gamma, clahe, brightness, sharpen
- class_imbalance severity=medium suggestions=class-aware-sampling, targeted-augmentation, class-balanced-policy
- high_false_positive severity=medium suggestions=hard-negative-review, reduce-noise, reduce-blur, mild-lighting
- localization_bias severity=medium suggestions=mild-scale, mild-translate, reduce-rotate, reduce-shear, reduce-perspective

## Per-Class Summary
- OK2: gt=64 tp=64 fp=21 fn=0 precision=0.7529 recall=1.0000
- OK3: gt=274 tp=274 fp=68 fn=0 precision=0.8012 recall=1.0000
- 加强筋打伤: gt=8 tp=7 fp=0 fn=1 precision=1.0000 recall=0.8750
- 开裂: gt=2 tp=2 fp=8 fn=0 precision=0.2000 recall=1.0000
- 油污: gt=18 tp=5 fp=30 fn=12 precision=0.1429 recall=0.2778
- 浅划伤: gt=13 tp=7 fp=4 fn=6 precision=0.6364 recall=0.5385
- 漏背锡: gt=46 tp=26 fp=12 fn=18 precision=0.6842 recall=0.5652
- 碰伤: gt=269 tp=196 fp=98 fn=65 precision=0.6667 recall=0.7286
- 脏污: gt=42 tp=22 fp=17 fn=15 precision=0.5641 recall=0.5238
- 轮廓划伤: gt=84 tp=50 fp=33 fn=30 precision=0.6024 recall=0.5952
- 锡丝残留: gt=12 tp=10 fp=7 fn=2 precision=0.5882 recall=0.8333
- 锡尖: gt=27 tp=25 fp=4 fn=2 precision=0.8621 recall=0.9259
- 锡膏: gt=46 tp=36 fp=24 fn=10 precision=0.6000 recall=0.7826
