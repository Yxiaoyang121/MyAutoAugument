# Validation Error Diagnosis

- Status: completed
- TP: 720
- FP: 383
- FN: 170
- Precision: 0.6528
- Recall: 0.7956

## Diagnosis Vector
- small_object_score: 0.2683
- low_contrast_score: 0.5488
- class_imbalance_score: 0.8876
- localization_score: 0.0166
- false_positive_score: 0.3472

## Issues
- low_contrast_missed_defect severity=high suggestions=contrast, gamma, clahe, brightness, sharpen
- low_contrast_missed_defect severity=medium suggestions=contrast, gamma, clahe, brightness, sharpen
- class_imbalance severity=medium suggestions=class-aware-sampling, targeted-augmentation, class-balanced-policy
- high_false_positive severity=medium suggestions=hard-negative-review, reduce-noise, reduce-blur, mild-lighting
- localization_bias severity=medium suggestions=mild-scale, mild-translate, reduce-rotate, reduce-shear, reduce-perspective

## Per-Class Summary
- OK2: gt=64 tp=64 fp=20 fn=0 precision=0.7619 recall=1.0000
- OK3: gt=274 tp=274 fp=68 fn=0 precision=0.8012 recall=1.0000
- 加强筋打伤: gt=8 tp=7 fp=1 fn=1 precision=0.8750 recall=0.8750
- 开裂: gt=2 tp=2 fp=10 fn=0 precision=0.1667 recall=1.0000
- 油污: gt=18 tp=10 fp=75 fn=8 precision=0.1176 recall=0.5556
- 浅划伤: gt=13 tp=4 fp=0 fn=9 precision=1.0000 recall=0.3077
- 漏背锡: gt=46 tp=39 fp=38 fn=5 precision=0.5065 recall=0.8478
- 碰伤: gt=269 tp=178 fp=63 fn=85 precision=0.7386 recall=0.6617
- 脏污: gt=42 tp=22 fp=48 fn=16 precision=0.3143 recall=0.5238
- 轮廓划伤: gt=84 tp=54 fp=30 fn=28 precision=0.6429 recall=0.6429
- 锡丝残留: gt=12 tp=9 fp=11 fn=3 precision=0.4500 recall=0.7500
- 锡尖: gt=27 tp=25 fp=4 fn=2 precision=0.8621 recall=0.9259
- 锡膏: gt=46 tp=32 fp=15 fn=13 precision=0.6809 recall=0.6957
