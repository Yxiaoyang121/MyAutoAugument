# Validation Error Diagnosis

- Status: completed
- TP: 747
- FP: 373
- FN: 145
- Precision: 0.6670
- Recall: 0.8254

## Diagnosis Vector
- small_object_score: 0.2156
- low_contrast_score: 0.5000
- class_imbalance_score: 0.8397
- localization_score: 0.0144
- false_positive_score: 0.3330

## Issues
- low_contrast_missed_defect severity=medium suggestions=contrast, gamma, clahe, brightness, sharpen
- low_contrast_missed_defect severity=medium suggestions=contrast, gamma, clahe, brightness, sharpen
- class_imbalance severity=medium suggestions=class-aware-sampling, targeted-augmentation, class-balanced-policy
- high_false_positive severity=medium suggestions=hard-negative-review, reduce-noise, reduce-blur, mild-lighting
- localization_bias severity=medium suggestions=mild-scale, mild-translate, reduce-rotate, reduce-shear, reduce-perspective

## Per-Class Summary
- OK2: gt=64 tp=64 fp=20 fn=0 precision=0.7619 recall=1.0000
- OK3: gt=274 tp=274 fp=69 fn=0 precision=0.7988 recall=1.0000
- 加强筋打伤: gt=8 tp=7 fp=2 fn=1 precision=0.7778 recall=0.8750
- 开裂: gt=2 tp=1 fp=10 fn=1 precision=0.0909 recall=0.5000
- 油污: gt=18 tp=8 fp=48 fn=10 precision=0.1429 recall=0.4444
- 浅划伤: gt=13 tp=6 fp=0 fn=7 precision=1.0000 recall=0.4615
- 漏背锡: gt=46 tp=36 fp=31 fn=9 precision=0.5373 recall=0.7826
- 碰伤: gt=269 tp=202 fp=77 fn=61 precision=0.7240 recall=0.7509
- 脏污: gt=42 tp=21 fp=25 fn=17 precision=0.4565 recall=0.5000
- 轮廓划伤: gt=84 tp=55 fp=37 fn=28 precision=0.5978 recall=0.6548
- 锡丝残留: gt=12 tp=9 fp=8 fn=3 precision=0.5294 recall=0.7500
- 锡尖: gt=27 tp=27 fp=8 fn=0 precision=0.7714 recall=1.0000
- 锡膏: gt=46 tp=37 fp=38 fn=8 precision=0.4933 recall=0.8043
