# Validation Error Diagnosis

- Status: completed
- TP: 695
- FP: 332
- FN: 201
- Precision: 0.6767
- Recall: 0.7680

## Diagnosis Vector
- small_object_score: 0.2818
- low_contrast_score: 0.4960
- class_imbalance_score: 0.9953
- localization_score: 0.0099
- false_positive_score: 0.3233

## Issues
- low_contrast_missed_defect severity=medium suggestions=contrast, gamma, clahe, brightness, sharpen
- low_contrast_missed_defect severity=medium suggestions=contrast, gamma, clahe, brightness, sharpen
- class_imbalance severity=medium suggestions=class-aware-sampling, targeted-augmentation, class-balanced-policy
- high_false_positive severity=medium suggestions=hard-negative-review, reduce-noise, reduce-blur, mild-lighting
- localization_bias severity=medium suggestions=mild-scale, mild-translate, reduce-rotate, reduce-shear, reduce-perspective

## Per-Class Summary
- OK2: gt=64 tp=64 fp=26 fn=0 precision=0.7111 recall=1.0000
- OK3: gt=274 tp=274 fp=71 fn=0 precision=0.7942 recall=1.0000
- 加强筋打伤: gt=8 tp=7 fp=1 fn=1 precision=0.8750 recall=0.8750
- 开裂: gt=2 tp=0 fp=4 fn=2 precision=0.0000 recall=0.0000
- 油污: gt=18 tp=9 fp=53 fn=9 precision=0.1452 recall=0.5000
- 浅划伤: gt=13 tp=6 fp=10 fn=7 precision=0.3750 recall=0.4615
- 漏背锡: gt=46 tp=23 fp=3 fn=21 precision=0.8846 recall=0.5000
- 碰伤: gt=269 tp=180 fp=78 fn=84 precision=0.6977 recall=0.6691
- 脏污: gt=42 tp=20 fp=23 fn=20 precision=0.4651 recall=0.4762
- 轮廓划伤: gt=84 tp=55 fp=41 fn=29 precision=0.5729 recall=0.6548
- 锡丝残留: gt=12 tp=9 fp=8 fn=3 precision=0.5294 recall=0.7500
- 锡尖: gt=27 tp=27 fp=6 fn=0 precision=0.8182 recall=1.0000
- 锡膏: gt=46 tp=21 fp=8 fn=25 precision=0.7241 recall=0.4565
